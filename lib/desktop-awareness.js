...
function createDesktopAwarenessEngine({
    ...
    async function handleFrame(frame) {
        ...
        // 只有通过全部门禁、即将真正调用视觉模型的截图才进入思考状态。
        // 固定间隔采样和右键强制测试共用此入口，避免两条路径的动作反馈不一致。
        emitPetEvent('thinking', {
            activity: 'desktop-analysis',
            forced: Boolean(frame.force),
            priority: 30,
        });
        try {
            ...
            processing = false;
            // 无论模型回复、静默、解析失败还是抛错，都必须结束循环思考动作。
            emitPetEvent('desktopAnalysisFinished', {
                activity: 'desktop-analysis',
                forced: Boolean(frame.force),
            });
        }
