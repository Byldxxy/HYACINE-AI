...
function observeMessage(groupId, senderName, text) {
    ...
    // 最多保留最近30条
    if (recentObservations[groupId].length > 30) {
        recentObservations[groupId] = recentObservations[groupId].slice(-30);
    }
}
...
async function proactiveCheck(deps) {
    ...
    for (const groupId of Object.keys(recentObservations)) {
        ...
        const obs = recentObservations[groupId];
...
module.exports = { observeMessage, proactiveCheck, restartProactiveTimer, getProactiveState };
