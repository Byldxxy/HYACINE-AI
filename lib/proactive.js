 ...
 * 群聊主动发言引擎。
 *
 * 普通群消息先由 message-handler 调用 observeMessage 写入内存观察缓冲区；定时器只在
 * 指定群、满足最少上下文和冷却条件时请求模型评分。观察缓冲不写盘，真正发出的回复
 * 才进入会话记忆。该模块与桌面感知是两套独立的主动机制。
 */
...
function parseDecision(content, fallbackThreshold) {
    // 首选约定的 JSON；部分兼容模型仍会直接返回句子，此时按阈值视为可发送候选。
...
async function proactiveCheck(deps) {
    ...
    // 按群依次判断，避免多个群同时发起模型请求或同时占用同一个 NapCat 连接。
    for (const groupId of Object.keys(recentObservations)) {
        ...
        try {
            ...
            if (decision.score >= threshold && decision.reply.length > 0) {
                ...
                // 发送成功后清空观察窗口，防止下一次定时检查围绕同一话题重复插话。
                recentObservations[groupId] = [];
            }
        } catch (e) {
...
function restartProactiveTimer(config) {
    // config 可以是对象，也可以是返回最新依赖的函数。server.js 使用函数形式，
    // 因此定时器每次触发都能看到 WebUI 刚保存的新配置和最新连接列表。
