...
const { buildVisionContent, extractImageSources, prepareVisionImages, stripImageSegments } = require('./vision');
...
function createMessageHandler(deps) {
    ...
    `;
        ...
        try {
            ...
            const currentUserIndex = sessions[sessionId].length - 1;
            messagesToSend.push(...sessions[sessionId].map((message, index) => {
                if (index !== currentUserIndex || message.role !== 'user' || visionImages.length === 0) {
                    return message;
                }
                return {
                    ...message,
                    content: buildVisionContent(message.content, visionImages),
                };
            }));
